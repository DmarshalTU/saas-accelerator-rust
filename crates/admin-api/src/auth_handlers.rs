...
pub async fn callback_handler(
    ...
    // The token response may contain id_token in extra_fields
    // For now, we'll extract it from the access token or use a different approach
    // Note: Azure AD v2.0 endpoint returns ID token in the token response
    // We need to parse the response JSON directly to get the id_token
    let token_response_body = format!("{:?}", token_result);
    error!("Token response: {}", token_response_body);
    
    // For Azure AD, the ID token is typically in the response
    // We'll need to handle this differently - Azure AD OpenID Connect returns id_token
    // in the token endpoint response as a JSON field
    // This is a temporary workaround - we should parse the actual response
