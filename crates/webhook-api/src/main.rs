...
use shared::services::{
    SubscriptionServiceTrait, SubscriptionServiceImpl,
    ApplicationLogServiceTrait, ApplicationLogServiceImpl,
    SubscriptionStatusHandler, NotificationStatusHandler, EmailServiceTrait, SmtpEmailService,
    EmailHelper, AbstractSubscriptionStatusHandler,
};
use std::sync::Arc;
...
struct AppState {
    ...
    jwt_validator: Arc<shared::auth::JwtValidator>,
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Starting Webhook API server");
    ...
    let subscription_service: Arc<dyn SubscriptionServiceTrait> = Arc::new(
        SubscriptionServiceImpl::new(
            subscription_repo_adapter,
            plan_repo_adapter,
            0,
        )
    );
    let application_log_service: Arc<dyn ApplicationLogServiceTrait> = Arc::new(
        ApplicationLogServiceImpl::new(
            application_log_repo_adapter,
        )
    );

    let application_log_service_for_email: Arc<dyn shared::services::email_service::ApplicationLogServiceForEmail> =
        Arc::new(adapters::ApplicationLogServiceAdapterForEmail::new(application_log_service.clone()));

    let email_service: Arc<dyn EmailServiceTrait> = Arc::new(
        SmtpEmailService::new(
            application_log_service_for_email,
        )
    );
    ...
    let plan_events_mapping_repo_for_email_helper: Arc<dyn shared::services::email_helper::PlanEventsMappingRepositoryForEmailHelper> =
        Arc::new(adapters::PlanEventsMappingRepositoryAdapter::new(plan_events_mapping_repo.clone()));

    let email_helper: Arc<EmailHelper> = Arc::new(
        EmailHelper::new(
    ...
    let subscription_repo_for_status_handler: Arc<dyn shared::services::status_handlers::SubscriptionRepositoryHelper> =
        Arc::new(adapters::SubscriptionRepositoryAdapterForStatusHandler::new(subscription_repo.clone()));
    ...
    let user_repo_for_status_handler: Arc<dyn shared::services::status_handlers::UserRepositoryHelper> =
        Arc::new(adapters::UserRepositoryAdapterForStatusHandler::new(user_repo.clone()));
    ...
    let config_repo_for_notification: Arc<dyn shared::services::notification_status_handler::ApplicationConfigRepositoryForNotification> =
        Arc::new(adapters::ApplicationConfigRepositoryAdapterForNotification::new(config_repo.clone()));
...
async fn handle_webhook(
    ...
    let result = match payload.action {
        ...
        shared::models::WebhookAction::Renew => {
            state.webhook_handler.handle_renew(&payload).await
        }
